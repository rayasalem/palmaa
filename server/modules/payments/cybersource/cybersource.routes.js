...
import {
  createHostedSessionHandler,
  notificationHandler,
} from './cybersource.controller.js';
...
router.all('/rest/process', (req, res) => res.status(405).set('Allow', 'POST').json({ error: 'Method not allowed. Use POST.' }));
router.all('/rest/test', (req, res) => res.status(405).set('Allow', 'POST').json({ error: 'Method not allowed. Use POST.' }));
