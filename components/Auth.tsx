...
export const Auth: React.FC<AuthProps> = ({ onLogin, initialView = 'LOGIN', onOpenTerms }) => {
  ...
  useEffect(() => {
    if (initialView === 'REGISTER_STUDENT') {
      ...
      ['LOGIN', 'ROLE_SELECT', 'REGISTER_MERCHANT', 'REGISTER_BROKER', 'REGISTER_CUSTOMER'].includes(initialView)
    ) {
  ...
  if (view === 'REGISTER_CUSTOMER')
    return <RegisterCustomer onRegister={onLogin} onBackToLogin={() => setView('LOGIN')} />;
  if (view === 'REGISTER_MERCHANT')
    return <RegisterMerchant onRegister={onLogin} onBackToLogin={() => setView('LOGIN')} onOpenTerms={onOpenTerms} />;

  return (
    <div
      className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 sm:p-6 font-sans font-heading"
      dir={lang === 'en' ? 'ltr' : 'rtl'}
    >
        ...
        <div className="bg-white rounded-2xl shadow-card border border-palma-border overflow-hidden">
          ...
          <div className="p-8 sm:p-10 pt-4">
            <div className="heading-block mb-10">
              ...
              <p className="heading-block-sub">{view === 'LOGIN' ? t.auth.digitalJourney : t.auth.roleSubtitle}</p>
            </div>
                    ...
                    onSubmit={async (e) => {
                      ...
                      if (result.success) {
                        ...
                        if (code) {
                          ...
                          showToast(
                            lang === 'ar' ? 'تم إرسال رمز التحقق إلى بريدك' : 'Check your email for the reset code',
                            'success'
                          );
                          setForgotOtp('');
                        }
                        ...
                        const err =
                          typeof result.error === 'string'
                            ? result.error
                            : (result.error as any)?.message || 'Request failed';
            ...
            {view === 'LOGIN' && !showForgotPassword && !verificationMode && (
              <form onSubmit={handleLogin} className="space-y-6 animate-fade-in">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="btn-primary w-full py-5 text-sm tracking-wide shadow-soft hover:shadow-card-hover active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-3 mt-4"
                >
                  {loading ? (
                    <>
              ...
              <div className="space-y-8 animate-fade-in">
                <div className="grid grid-cols-2 gap-4">
                  {roleOptions.map((r) => (
                    ...
                    >
                      <div
                        className={`p-3 rounded-xl transition-colors duration-200 ${role === r.id ? 'bg-palma-primary text-white shadow-soft' : 'bg-white text-slate-400 group-hover:text-palma-primary shadow-soft border border-palma-border'}`}
                      >
                        {r.icon}
                      </div>
                      <div>
                        <span
                          className={`block text-xs font-black uppercase tracking-wider mb-1 ${role === r.id ? 'text-palma-primary' : 'text-palma-navy'}`}
                        >
                          {r.label}
                        </span>
