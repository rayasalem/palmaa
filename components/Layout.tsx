...
const Layout: React.FC<LayoutProps> = ({
    ...
    user.role === Role.MERCHANT
      ...
      : user.role === Role.BROKER
        ? [
            {
              id: 'subscription',
              label: lang === 'ar' ? 'باقة الاشتراك' : lang === 'he' ? 'חבילת מנוי' : 'Subscription',
              icon: 'FileText',
            },
  ...
  const firstTabId =
    userMenuItems[0]?.id ?? (user.role === Role.ADMIN ? 'users' : user.role === Role.CUSTOMER ? 'home' : 'dashboard');

  return (
    <div
      className="min-h-screen flex flex-col bg-[#f8f7fa] font-sans text-palma-text font-heading"
      dir={lang === 'en' ? 'ltr' : 'rtl'}
    >
            ...
            <div className="flex items-center gap-3 sm:gap-6 min-w-0">
              <div className="lg:hidden shrink-0">
                <button
                  ...
                  className="p-2.5 text-palma-navy hover:bg-slate-100 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-palma-primary/20"
                  aria-label="Menu"
                >
              ...
              <div
                className="cursor-pointer flex items-center hover:opacity-90 transition-opacity shrink-0"
                onClick={() => onTabChange(firstTabId)}
              >
              ...
              <div className="hidden sm:flex items-center gap-2 text-sm font-bold text-palma-muted overflow-hidden min-w-0">
                ...
                <span className="truncate flex items-center gap-1.5">
                  <button
                    type="button"
                    onClick={() => onTabChange(firstTabId)}
                    className="hover:text-palma-primary transition-colors whitespace-nowrap"
                  >
                    {homeLabel}
                  </button>
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="hidden sm:block relative">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 shrink-0 transition-transform ${langMenuOpen ? 'rotate-180' : ''}`}
                  />
                </button>
                {langMenuOpen && (
                  <>
      ...
      <div className="flex flex-1 max-w-[1800px] mx-auto w-full">
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div
              className="absolute top-0 right-0 bottom-0 w-80 bg-white shadow-card-hover flex flex-col border-l border-palma-border"
            ...
            >
              <div className="p-6 border-b border-palma-border flex justify-between items-center bg-white rounded-tl-2xl">
                ...
                >
                  <X className="w-5 h-5 text-palma-muted" />
                </button>
              </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
                        activeTab === item.id
                          ...
                          : 'text-palma-muted hover:bg-slate-50'
                      }`}
                    >
                      <span className="inline-flex items-center justify-center w-5 h-5 shrink-0 text-current">
                        {getIcon(item.icon)}
                      </span>
                      {item.label}
                    </button>
              ...
              <div className="p-6 border-t border-palma-border bg-slate-50/80">
                <button
                  onClick={onLogout}
                  className="flex items-center gap-3 text-sm font-semibold text-red-600 w-full justify-center py-3 bg-white border border-palma-border rounded-xl hover:bg-red-50 hover:border-red-100 transition-all duration-200 shadow-soft"
                >
                  <LogOut size={18} />
                  {t.common.logout}
                </button>
        ...
        >
          <div className="flex justify-around items-center h-20 pb-2">
            {userMenuItems.map((item) => (
              ...
              >
                <div
                  className={`p-2 rounded-xl transition-all duration-200 flex items-center justify-center w-9 h-9 shrink-0 ${activeTab === item.id ? 'bg-palma-primaryLight -translate-y-0.5' : 'group-hover:bg-slate-50'}`}
                >
