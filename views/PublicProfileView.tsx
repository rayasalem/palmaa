...
import { ArrowLeft, UserPlus, Check, MapPin, Store, TrendingUp, Globe, ChevronDown } from 'lucide-react';
...
const PublicProfileView: React.FC<PublicProfileViewProps> = ({
  ...
  useEffect(() => {
    ...
    if (local) {
      ...
      const localProducts = marketStore
        .getProducts()
        .filter((p) => p.merchant_id === profileId || p.merchantId === profileId);
  ...
  const profileImg =
    ...
    profileUser.logoUrl ||
    `https://ui-avatars.com/api/?name=${profileUser.name}&background=1F5D42&color=fff&size=200`;
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900" dir={lang === 'en' ? 'ltr' : 'rtl'}>
      ...
      {currentUser && (
        <div className="p-4 sm:p-8 max-w-7xl mx-auto">
          <button
            onClick={onBack}
            className="flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-palma-navy mb-4"
          >
      ...
      <main className="pb-32 px-4 sm:px-6 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h1 className="font-heading text-3xl font-black text-palma-navy leading-tight">
                        {profileUser.name}
                      </h1>
                      {profileUser.isApproved && (
                    ...
                    <div className="flex flex-wrap gap-4 text-xs font-bold text-slate-500">
                      <span className="flex items-center gap-1.5">
                        ...
                        {profileUser.role === Role.MERCHANT ? 'Merchant Store' : 'Broker Portfolio'}
                      </span>
                      {profileUser.city && (
                        <span className="flex items-center gap-1.5">
                      ...
                      <span className="text-palma-primary">
                        {followerCount} {lang === 'en' ? 'Followers' : 'متابع'}
                      </span>
                    </div>
                  </div>
        ...
        {profileUser.role === Role.MERCHANT && (
          <div className="space-y-8">
            <h2 className="font-heading text-2xl font-black text-palma-navy px-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {products.map((p) => (
                  <div
                    ...
                    className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all cursor-pointer group hover:-translate-y-1"
                  >
        ...
        {profileUser.role === Role.BROKER && (
          <div className="space-y-8">
            <h2 className="font-heading text-2xl font-black text-palma-navy px-2 flex items-center gap-2">
              <TrendingUp className="w-6 h-6" /> {lang === 'en' ? 'Endorsed Products' : 'المنتجات الموصى بها'}
            </h2>
                ...
                {sharedProducts.map((s) => {
                  ...
                  return (
                    <div
                      key={s.id}
                      onClick={() => onProductClick(p.id)}
                      className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-lg transition-all cursor-pointer flex gap-6 items-center"
                    >
                      ...
                      <div className="flex-1">
                        ...
                        {s.marketing_title && (
                          <p className="text-sm font-medium text-palma-primary italic">"{s.marketing_title}"</p>
                        )}
