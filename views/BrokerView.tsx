...
export const BrokerView: React.FC<Props> = ({ lang, user, onRefresh, activeTab, onTabChange, onViewProduct, onViewProfile }) => {
  ...
  const handleToggleFeatured = async (shareId: string) => {
    try {
      ...
      if (res.success && res.shared) {
        setSharedMeta(prev => prev.map(sp => sp.id === shareId ? { ...sp, is_featured: res.shared!.is_featured } : sp));
                ...
                <div key={p.id} className="bg-white rounded-[2.5rem] p-4 border border-palma-border group shadow-card hover:shadow-card-hover transition-all duration-300 card-hover-lift">
                  ...
                  <div className="px-2 space-y-3 mb-6">
                    <div>
                      ...
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); const mid = p.merchant_id || p.merchantId; if (mid && onViewProfile) onViewProfile(mid); }}
                        className="text-[9px] font-bold text-slate-400 uppercase tracking-widest hover:text-palma-primary hover:underline text-left"
                      >{t.common.merchantName}: {mName}</button>
                    </div>
                    <div className="flex justify-between items-center bg-slate-50 px-4 py-2 rounded-xl">
                      ...
                      <span className="text-sm font-black text-palma-primary">₪{((p.price || p.price_ils || 0) * 0.02).toFixed(1)}</span>
                    </div>
                  </div>
          ...
          {sharedMeta.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-8">
              {sharedMeta.slice().sort((a, b) => (b.is_featured ? 1 : 0) - (a.is_featured ? 1 : 0)).map(s => {
                ...
                const displayImage = p.images?.[0] || p.imageUrl || p.image_url || 'https://placehold.co/400x400?text=No+Image';
                return (
                  <div key={s.id} className={`bg-white rounded-[3.5rem] p-8 md:p-12 border-2 ${s.is_featured ? 'border-palma-primary shadow-2xl scale-[1.01]' : 'border-slate-50 shadow-sm'} flex flex-col md:flex-row gap-10 hover:shadow-xl transition-all group relative overflow-hidden`}>
                    ...
                    <div className="flex-1 flex flex-col justify-between space-y-6">
                      <div className="space-y-4">
                        <div className="flex justify-between items-start">
                          <div className="space-y-1">
                            ...
                            <button
                              type="button"
                              onClick={() => { const mid = p.merchant_id || p.merchantId; if (mid && onViewProfile) onViewProfile(mid); }}
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-6 pt-4 border-t border-slate-50">
                        ...
                        <div className="flex-1 w-full flex gap-3">
                           <button onClick={() => handleGenLink(p.id)} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-palma-primary transition-all shadow-lg active:scale-95">
                ...
                myCommissions.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-8 py-5">
                      <span className={`uppercase text-[9px] font-black px-2 py-1 rounded-lg ${c.status === 'PAID' ? 'bg-palma-primaryLight text-palma-primary' : 'bg-amber-50 text-amber-600'}`}>
                        {c.status}
                      </span>
                    </td>
                  </tr>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                       <div className="bg-palma-primary h-full rounded-full" style={{ width: `${user.clicks ? Math.min(100, (myCommissions.length / user.clicks) * 100) : 0}%` }}></div>
                    </div>
                 </div>
              </div>
