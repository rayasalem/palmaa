...
export default function AdminOffersTab() {
  ...
  const openEdit = (o: ShopOffer) => {
    ...
    setForm({
      ...
      scope: (o.scope as 'product' | 'category' | 'all') || (o.product_id ? 'product' : 'all'),
      category: o.category || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const scope = form.scope || (form.type === 'product' && form.product_id ? 'product' : 'all');
      const payload = {
        type: form.type,
        ...
        product_id: scope === 'product' && form.product_id ? form.product_id : null,
        is_active: form.is_active,
        scope,
        category: scope === 'category' && form.category ? form.category : null,
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <p className="mt-1.5 text-xs text-slate-500">
                {form.type === 'product'
                  ...
                  : (lang === 'ar' ? 'البطاقة المخصصة تظهر في المتجر؛ الخصم يُطبّق حسب «نطاق الخصم» أدناه.' : 'Custom card is shown in the shop; the discount applies to the cart according to "Discount scope" below.')}
              </p>
            </div>
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1">
                {lang === 'ar' ? 'نطاق الخصم على السلة' : 'Cart discount scope'}
              </label>
              <select
                className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold"
                value={form.scope}
            ...
            <div>
              ...
              <input
                ...
                placeholder="20"
              />
              {(form.scope === 'all' || form.scope === 'category' || form.scope === 'product') && form.discount_label > 0 && (
                <p className="mt-1 text-xs text-emerald-600 font-medium">
                  {lang === 'ar' ? 'سيُطبّق هذا الخصم على السلة حسب النطاق المحدد.' : 'This discount will be applied to the cart according to the scope selected.'}
                </p>
              )}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1">
                  {lang === 'ar' ? 'بداية العرض (اختياري)' : 'Offer start (optional)'}
                </label>
                <input
                  type="datetime-local"
                  className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold"
              ...
              <div>
                <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1">
                  {lang === 'ar' ? 'نهاية العرض (اختياري)' : 'Offer end (optional)'}
                </label>
                <input
                  type="datetime-local"
                  className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold"
            ...
            <p className="text-xs text-slate-500">
              {lang === 'ar' ? 'اتركهما فارغين ليكون العرض دائماً فعّالاً ضمن فترة النشاط.' : 'Leave empty for the offer to be always active within the active period.'}
            </p>
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1">
          ...
          {offers.map((o) => (
            ...
            >
              ...
              {o.scope === 'category' && o.category && (
                <p className="text-xs text-slate-400 truncate">{o.category}</p>
              )}
              {o.scope === 'product' && o.product_id && (
