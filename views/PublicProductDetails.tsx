...
const PublicProductDetails: React.FC<PublicProductDetailsProps> = ({ lang, user, productId, onBack, onLoginClick, onRefresh, onViewProfile, addToCart }) => {
  ...
  useEffect(() => {
    if (!productId) return;
    getProductLikesCount(productId).then((r) => setLikesCount(r.count ?? 0)).catch(() => setLikesCount(0));
    getProductComments(productId)
      ...
      .catch(() => setComments([]));
    if (user) {
      getProductIsLiked(productId).then((r) => setIsLiked(r.liked ?? false)).catch(() => setIsLiked(false));
    } else {
      setIsLiked(false);
    }
  }, [productId, user?.id]);
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(e?.data?.error || e?.message || (lang === 'en' ? 'Failed to add comment' : 'فشل إضافة التعليق'), 'error');
    } finally {
                 ...
                 <div className="relative z-10 space-y-8">
                    ...
                    >
                        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-sm overflow-hidden border border-slate-100 shrink-0">
                           <img src={merchantProfile?.logo_url || `https://ui-avatars.com/api/?name=${merchantName}&background=random`} className="w-full h-full object-cover" alt="" />
                        </div>
                        <div className="flex-1 min-w-0">
                           <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest">{t.common.merchantName}</p>
                           <p className="text-sm font-bold text-palma-navy truncate">{merchantName || (lang === 'ar' ? 'التاجر' : 'Merchant')}</p>
                           {onViewProfile && (product.merchant_id || product.merchantId) && (
                             <p className="text-[9px] font-bold text-palma-primary mt-0.5">{lang === 'ar' ? 'عرض صفحة التاجر ←' : 'View merchant page →'}</p>
                           )}
                        </div>
                    </button>
                 ...
                 <h3 className="text-xl font-black text-palma-navy flex items-center gap-3">
                  ...
                  {user && (
                    <form onSubmit={handleAddComment} className="flex gap-3">
                       ...
                       />
                       <button type="submit" className="bg-palma-navy text-white p-3 rounded-xl hover:bg-palma-primary transition-all shadow-md disabled:opacity-50" disabled={!socialCommentInput.trim() || commentLoading}>
                       ...
                       comments.map(c => (
                          <div key={c.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                             <div className="flex justify-between items-start mb-2">
                                <span className="text-xs font-black text-palma-navy">{c.userName ?? (user && c.userId === user.id ? user.name : 'User')}</span>
