  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button type="button" onClick={onRefreshOrders} className="flex items-center gap-2 text-sm font-bold text-palma-primary hover:text-palma-navy transition-colors">
                      ...
                      return (
                        <div key={item.id || `oi-${idx}`} className="flex items-center gap-4">
                          <div className="w-14 h-14 rounded-xl bg-slate-50 border border-slate-100 overflow-hidden shrink-0">
                            <img src={prod?.images?.[0] || prod?.imageUrl || 'https://placehold.co/100x100?text=No+Image'} loading="lazy" className="w-full h-full object-cover" alt="" />
                          </div>
