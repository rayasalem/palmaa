...
- **الحل:** لا تضف في `server/.env` أي متغيرات لـ outlet/terminal للاختبار (اترك `CYBS_ILS_OUTLET_ID` و `CYBS_ILS_TERMINAL_ID` وغيرها **غير معرّفة**). الكود **لا يرسل** هذه الحقول إلا إذا وُجدت قيم حقيقية (ولا يرسل قيماً وهمية مثل TEST\_\*)، وبالتالي لن يظهر خطأ 150 بسببها.
