...
| البند | الوضع |
|-------|--------|
| **طريقة الدفع في الموقع** | Hosted Checkout (العميل يضغط "متابعة للدفع" → صفحة الدفع والشحن → ثم يُحوّل لصفحة Cybersource لإدخال البطاقة). |
...
- فعّل حقل **Country** (Display/Edit/Require حسب الحاجة) حتى يرسل العميل أو النظام `bill_to_address_country`.
- إذا أردت أن يملأها موقعك: أضف في الطلب من الباكند حقل `bill_to_address_country` (مثلاً من عنوان العميل) في جلسة Hosted؛ الكود الحالي لا يرسلها — يمكن إضافتها لاحقاً إن طلب البنك.
